...
`fileWatcher.ts` is the core loop: on startup and on every chokidar change event, it backfills any not-yet-completed rotated `*.log.gz` file (read once, whole file, marked completed) and incrementally tails `latest.log` from a stored byte offset. Per-file progress lives in the `ingest_checkpoints` table (`db/schema.ts`), keyed by filename with an inode check that detects log rotation (rename + recreate). Rotation is a *handover*, not just a reset: the lines already live-tailed sit in `raw_lines` under the `latest.log` file_key, and the rotated `.gz` that appears contains those same lines — so `resolveRotation` (which must run before the rotated-file backfill loop) identifies the rotated file by matching the first/last tailed line's redacted content, re-keys the existing rows to that filename, and leaves a non-completed checkpoint at the tailed line count so `processRotatedFile` resumes *after* them (picking up only the tail written between the last sweep and rotation). Skipping either half of this is a real, previously-shipped bug: without the resume, the `.gz` backfill re-ingested everything under its own file_key (`raw_lines`' `(file_key, line_no)` dedupe can't catch a changed file_key, and `events` has no dedupe), duplicating every already-tailed POI after each rotation; and without the re-key, the new `latest.log` generation's line numbers collided with the old generation's rows, so `INSERT OR IGNORE` silently attached new events to stale raw lines. If the rotated file hasn't appeared yet when the inode change is noticed (mid-rotation race), the sweep defers tailing the new `latest.log` for up to 30s before parking the old rows under a synthetic `latest.log.orphaned.*` key.
